...
const config = {
  ...
  width: W,
  height: H,
  backgroundColor: "#000000", // fondo muy oscuro
  physics: {
    default: "arcade",
    arcade: {
      gravity: { y: 0 },
...
function preload() {
  ...
  // Gato vivo (negro con ojos blancos para visibilidad)
  g.clear();
  g.fillStyle(0x000000, 1); // cuerpo/rostro negro
  g.fillCircle(16, 18, 14);
  g.fillStyle(0x000000, 1); // orejas negras
  g.fillTriangle(6, 8, 12, 2, 14, 12);
  g.fillTriangle(18, 12, 20, 2, 26, 8);
  g.fillStyle(0xffffff, 1); // ojos blancos
  g.fillCircle(12, 18, 2);
  g.fillCircle(20, 18, 2);
  g.generateTexture("gatoVivo", 32, 32);

  // Gato muerto (gris oscuro con X rojas)
  g.clear();
  ...
  // Corazón visible (rojo fuerte)
  g.clear();
  g.fillStyle(0xff1e3a, 1);
...
function create() {
  // Fondo y capa sutil para contraste
  this.add.rectangle(W / 2, H / 2, W, H, 0x000000);
  this.add.rectangle(W / 2, H / 2, W, H, 0x0a0a0a).setAlpha(0.4);
  ...
  // UI
  livesText = this.add.text(10, 10, `Vidas: ${lives}`, { fontFamily: "monospace", fontSize: "14px", color: "#cccccc" }).setScrollFactor(0);
  timerText = this.add.text(10, 28, `Tiempo: 0.0 s`, { fontFamily: "monospace", fontSize: "14px", color: "#a0ffd6" }).setScrollFactor(0);
  infoText = this.add.text(W - 10, 10, `Reiniciar: R`, { fontFamily: "monospace", fontSize: "14px", color: "#cccccc" }).setOrigin(1, 0).setScrollFactor(0);
...
function onHit() {
  ...
  const stats = this.add.text(W / 2, H / 2 + 10, `Tiempo de esta vida: ${lastRunTime.toFixed(1)} s`, { fontFamily: "monospace", fontSize: "16px", color: "#cccccc" }).setOrigin(0.5);
...
function showGameOver() {
  ...
  const sumText = this.add.text(W / 2, H / 2 + 2, `Total sobrevivido: ${totalSurvival.toFixed(1)} s`, { fontFamily: "monospace", fontSize: "18px", color: "#cccccc" }).setOrigin(0.5);
